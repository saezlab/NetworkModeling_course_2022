...
This vignette contains the material for the first practical session of the Network Analysis course. It was developed by the [Saez-Rodriguez group members](https://saezlab.org/). The estimated required time to go over the content of the tutorial is of ~ 3 hours. This is the schedule for the practical session:
...
Next, we can now explore how the first rows of the counts matrix looks like. In this matrix, rows correspond to genes (here, labelled by their [HUGO gene symbol](https://www.genenames.org/)) and columns represent samples from which the expression data was measured.

```{r}
head(counts)
```

We can also explore the content of the metadata table, which contain the properties/annotations of each sample. Additional information that can be found in this type of tables include whether data is paired or not, or for example known batch effects, such as different sequencing lanes. Very often, this information is not explicitly provided but can be inferred from the sample name: e.g. PATIENT1_TUMOR_BATCH1 vs PATIENT2_NORMAL_BATCH2. 
...
**NOTE**: Inside this code chunk, we employ several functions. First, we create a data frame with the id of the samples and the total number of counts for each of them using the `data.frame()` and the `colSums()` functions. Next, we pass this data frame object to the `ggplot()` function to start creating the plot. This is done thanks to the pipe operand `%>%`, which passes the data frame created before it to to the function positioned after. For more information about the pipe operand please see [this documentation page](https://style.tidyverse.org/pipes.html). 

**NOTE**: The ggplot2 package, which is part of the `tidyverse` collection, is here employed to create the plot. ggplot2 comprises a powerful yet simple framework to create and modify high-quality graphics. For more information, please see [ggplot2 homepage](https://ggplot2.tidyverse.org/).
...
As it can be observed, a huge proportion of the counts with few counts were filtered. To aid visualization, we log transformed raw counts.
